...
void function3(int g[][G_SIZE], int d[][G_SIZE], int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            //ziyaret ettiðim j 0'a eþit veya j'nin mesafesi minimum mesafeden küçükse
			{
                minDistance = distance[j]; //minumum mesafeyi j'nin mesafesi yap
                minIndex = j; 
            }
        }
        
        if (minIndex == -1) {
            break; // Tüm düðümler ziyaret edilmiþse sonlandýr
        }
        
        visited[minIndex] = 1; //Ziyaret edilmesi gereken minumum index 1'e eþitse düðümü ziyaret et
        
        // Komþu düðümlerin mesafelerini güncelle
        for (int j = 0; j < size; j++) {
            if (g[minIndex][j] != 0 && visited[j] == 0) {
                int newDistance = distance[minIndex] + g[minIndex][j];
                if (newDistance < distance[j]) { //yeni mesafemiz j'nin mesafesinden küçükse 
                    distance[j] = newDistance; // j'nin mesafesini yeni mesafeye ata
                }
            }
        }
    }
    
    // Elde edilen mesafeleri d dizisine aktar
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (i == j) {
                d[i][j] = 0;
